...
import { analyzeLabText } from "../agents/analysisAgent.ts";
...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname === "/upload") {
    try {
      ...
      // Analyze using ClinSynapse
const { extractedText, ...result } = await analyzeLabText(tempFilePath);

// --- Auto-logging to ~/ClinSynapse/logs ---
try {
